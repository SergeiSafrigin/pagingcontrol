...
function init() {
	...
	if (args.tabs) {
		...
		$.tabsCtrl = Widget.createController('tabs', {
			...
			icons : _.map($.scrollableView.getViews(), function(v) {
				return v.icon;
			}),
			expand: args.expand,
	...
	if (args.currentTab) {
		var page = args.rtl ? $.scrollableView.getViews().length - args.currentTab : parseInt(args.currentTab - 1);
		$.scrollableView.setCurrentPage(page);
		var indicatorOffset = args.rtl? $.displayWidth - ($.iWidth * ($.scrollableView.views.length - page)) : page * $.iWidth;
		$.indicator.setLeft(indicatorOffset);
	}
...
function onScroll(e) {
	...
	var indicatorOffset = args.rtl? $.displayWidth - ($.iWidth * ($.scrollableView.views.length - e.currentPageAsFloat)) : e.currentPageAsFloat * $.iWidth;
	$.indicator.setLeft(indicatorOffset);
...
function onOrientationChange(e) {
	calcDisplayWidth();
	$.tabsCtrl.onOrientationChange(e);
...
function adjustePositions() {
	...
	var indicatorOffset = args.rtl ? $.displayWidth - (($.scrollableView.getCurrentPage() + 1) * $.iWidth) : $.scrollableView.getCurrentPage() * $.iWidth;
	$.indicator.setLeft(indicatorOffset);
}

function calcDisplayWidth() {
	$.displayWidth = OS_ANDROID ? Ti.Platform.displayCaps.platformWidth / Ti.Platform.displayCaps.logicalDensityFactor : Ti.Platform.displayCaps.platformWidth;
}
...
exports.setScrollableView = function(_sv) {
	...
	var views = _sv.getViews();
	
	_.each(views, function(view) {
		view.setBubbleParent(false);
	});
